...
**Sources:** [Product Vision](./001-product-vision.md) · [AGENTS.md](../../AGENTS.md) · [AT Protocol KB v2](../../.agents/kb/at-protocol-v2.md) · [AT Protocol KB](../../.agents/kb/at-protocol.md) · [HappyView KB](../../.agents/kb/happyview.md) · [C2PA KB](../../.agents/kb/c2pa.md)

> **Space URI note:** ATPix uses the **ATP-0016 proposal form** for permissioned spaces: `at://{spaceDid}/space/{spaceType}/{skey}` (records append `/{authorDid}/{collection}/{rkey}`). HappyView accepts this form and the legacy `ats://` dialect; ATPix **prefers and normalizes to** the proposal form ([ADR-010](../architecture/010-permissioned-spaces-storage.md)). Other HappyView dialect details (e.g. `mintPolicy`, App View space store) remain as implemented.
...
| **Private / permissioned album** | Permissioned **space repo** | `at://{spaceDid}/space/net.atpix.gallery.albumSpace/{skey}`; photos/items in space, not public index |
...
**Protocol mapping:** Permissioned **space repo** (`at://…/space/…`) for `photo` and `albumItem` **records**; album metadata **record** in public **repo** with `spaceUri`; space type `net.atpix.gallery.albumSpace`.
...
- Album owners MUST be able to designate an album with `visibility: permissioned` and receive a linked `spaceUri` (`at://{spaceDid}/space/net.atpix.gallery.albumSpace/{skey}`).
...
**Implemented In:** [SRS-F-001.4](./003-srs.md#srs-f-0014-hosted-pds-signup-discovery-link), [SRS-F-017](./003-srs.md#srs-f-017-hosted-pds-account-onboarding), [UI-SCR-009](./004-ui-requirements.md#ui-scr-009-sign-in-and-pds-onboarding), [ADR-006](../architecture/006-oauth-dpop-authentication.md)
...
**Implemented In:** [SRS-F-018](./003-srs.md#srs-f-018-embedded-signup-on-atpixnet-post-v1) (planned Task 9.1)
...
**Implemented In:** [SRS-F-019](./003-srs.md#srs-f-019-atpix-managed-pds-invites-post-v1) (planned Task 9.2)
...
**Implemented In:** [SRS-F-020](./003-srs.md#srs-f-020-apex-handle-provisioning-at-scale-post-v1) (planned Task 9.3)
...
**Implemented In:** [SRS-F-021](./003-srs.md#srs-f-021-entryway-and-multi-pds-federation-post-v1), [ADR-007](../architecture/007-happyview-app-view-integration.md) (planned Task 9.4)
...
**Requirement:** Blob bytes MUST reside in user PDS repositories (via `com.atproto.repo.uploadBlob`). Photo **records** for public/unlisted content MUST live in the user's public PDS repo as cryptographically signed atproto records. Photo and `albumItem` **records** for permissioned albums MUST live in the linked space repo (proposal-form `at://…/space/…` URIs); blobs remain on the author's PDS and are accessed via `com.atproto.space.getBlob` with membership gating. The App View MUST index and serve public copies but MUST NOT be the sole custodian of user media.  
...
| **Hosted PDS (operator)** | [Reference PDS](https://github.com/bluesky-social/pds) on OVH (`pds.atpix.net`) | `*.pds.atpix.net` self-service handles; separate from monorepo ([Phase B](../../README.md#phase-b--dedicated-pds-at-ovh), F-017–F-021) |
...
**Constraint:** Public and unlisted album photos MUST live in the user's public PDS repository. Permissioned album photos and their `albumItem` records MUST live in the associated Permissioned Space repo, addressed via proposal-form `at://{spaceDid}/space/…` URIs. Client-side encryption MUST NOT be implemented.  
...
| NSID | Purpose |
|------|---------|
| `net.atpix.gallery.albumSpace` | Permissioned Space **type** for gated albums (`at://…/space/net.atpix.gallery.albumSpace/{skey}`) |
...
| `spaceUri` | string | **ATPix** · ATP-0016 | Proposal-form `at://…/space/…` URI when permissioned |

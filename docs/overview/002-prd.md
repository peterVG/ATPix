...
> **Protocol dialect note:** Conceptual atproto and Permissioned Data (ATP-0016) rules are summarized in [at-protocol-v2.md](../../.agents/kb/at-protocol-v2.md). ATPix **v1 wire formats and storage paths follow HappyView experimental Spaces** ([happyview.dev.docs.md](../references/happyview.dev.docs.md), [ADR-010](../architecture/010-permissioned-spaces-storage.md)) — including the `ats://` URI scheme and HappyView-hosted space repos — which may differ from the Blacksky tutorial / proposal grammar (`at://…/space/…`). Do not “correct” PRD/SRS/Lexicon fields to the pure proposal without a deliberate HappyView-migration ADR.
